...
function AdminDashboard({ user }) {
  ...
  useEffect(() => {
    if (user?.role?.name === "Admin") fetchData();
  }, [user]);
  ...
  const updateBooking = (id) => {
    fetch(`/bookings/${id}/approve`, { method: "PATCH" }).then((r) => {
      if (r.ok) {
        setData((prev) => ({
          ...prev,
  ...
  const deleteHouse = (id) => {
    fetch(`/houses/${id}`, { method: "DELETE" }).then(() =>
      setData((prev) => ({
        ...prev,
          ...
          <div className="admin-scroll-box">
            {data.bookings.map((b) => (
              <div key={b.id} className="admin-card">
                <p>
                  <strong>{b.user?.username}</strong> → House #{b.house_id}
                </p>
                <p>
                  Status:{" "}
        ...
        <section className="admin-section full-width">
          ...
          <form onSubmit={handleHouseSubmit} className="admin-form">
            ...
            <div className="form-row">
              ...
              />
              <input
                placeholder="Type (Villa/Apt)"
                value={newHouse.house_type}
                onChange={(e) =>
                  setNewHouse({ ...newHouse, house_type: e.target.value })
                }
              />
              <input
                type="number"
                placeholder="Price"
                value={newHouse.price_per_night}
                onChange={(e) =>
                  setNewHouse({ ...newHouse, price_per_night: e.target.value })
                }
                required
              />
            </div>
            <input
              ...
              onChange={(e) =>
                setNewHouse({ ...newHouse, image_url: e.target.value })
              }
            />
            <textarea
            ...
            />
            <button type="submit">
              {editingHouse ? "Update Listing" : "Create Listing"}
            </button>
            {editingHouse && (
            ...
            {data.houses.map((h) => (
              <div key={h.id} className="admin-house-item">
                ...
                <div>
                  ...
                  <button className="btn-delete" onClick={() => deleteHouse(h.id)}>
                    Delete
                  </button>
                </div>
              </div>
